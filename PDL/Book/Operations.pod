...
I<broadcasted over>; see[sec:Dimensionality-and-Broadcasting]). Most 
...
other broadcasted Boolean expressions. The operators that return 
...
broadcasting over their arguments and returning a PDL with the same 
...
the broadcasting engine if there is sufficient demand. You might be 
...
dimensions must agree, broadcast-wise, between the original PDL and 
what you are appending to it (Remember the broadcasting rules from [sub:Broadcasting-rules]
?). So if you say:
