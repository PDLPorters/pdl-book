...
dimension at the beginning of the dim list, to a broadcast dimension 
...
around each pixel in the entire image, and then use the broadcasting 
...
PDL and returns them collated in a form suitable for broadcasting. 
...
broadcasted processing by a vectorized algorithm.
...
C<range> broadcasts over both C<$index> and C<$source>. The returned 
dimension list is stacked up like this:

  (index broadcast dims), (index dims (size)), (source broadcast dims)

The first few dims of the output correspond to the broadcast dims of 
C<$index> (beyond the 0 dim). They allow you to pick out individual 
ranges from a large, broadcasted collection, so that the output 
...
act like ordinary broadcast dims, because adding more dims to 
C<$source> just tacks extra dims on the end of the output. Each 
source broadcast dim ranges over the entire corresponding dim of 
C<$source>.
...
address broadcast dimensions, depending on your application. You can 
